...
(function() {
  ...
  var HorizontalTimeline = function(element) {
		...
		this.eventsMinDistance = 87; // min distance between two consecutive events (in px)
		this.eventsMaxDistance = 174; // max distance between two consecutive events (in px)
  ...
  function initTimeline(timeline) {
		// define initial left offset to center the first and last event evenly
		var left = timeline.eventsMinDistance / 2; // add half min distance as left margin
		var rightPadding = timeline.eventsMinDistance / 2;
		...
		for (var i = 0; i < timeline.dateValues.length; i++) {
			...
			timeline.date[i].style.left = left + "px";
			left = left + distanceNorm;
		}

		// total line length includes both side paddings
		var totalLength = left + rightPadding;
		timeline.line.style.width = totalLength + "px";
		timeline.lineLength = totalLength;

		// ensure line at least as wide as container
		if (containerWidth > timeline.lineLength) {
			timeline.line.style.width = containerWidth + "px";
			timeline.lineLength = containerWidth;
		}
  ...
  function updateFilling(timeline) {
		// get selected date's position
		var dateStyle = window.getComputedStyle(timeline.selectedDate, null),
				left = parseFloat(dateStyle.getPropertyValue("left")),
				width = parseFloat(dateStyle.getPropertyValue("width"));

		// if it's the last date, fill till end of the line
		if (timeline.newDateIndex === timeline.date.length - 1) {
			timeline.fillingLine.style.transform = 'scaleX(1)';
		} else {
			// normal behavior: fill till the selected date's center
			var fillPosition = left + width / 2;
			timeline.fillingLine.style.transform = 'scaleX(' + (fillPosition / timeline.lineLength) + ')';
		}
	}
  ...
  function translateTimeline(timeline, direction) { // translate timeline (and date elements)
    ...
    var oldEvent = timeline.content[timeline.oldDateIndex];
    var newEvent = timeline.content[timeline.newDateIndex];

    // fade out old content
    if (timeline.newDateIndex !== timeline.oldDateIndex) {
      oldEvent.style.opacity = 0;
      setTimeout(function() {
        oldEvent.classList.remove('h--timeline-event--selected');
    ...
    setTimeout(function() {
      newEvent.classList.add('h--timeline-event--selected');
